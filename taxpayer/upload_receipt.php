...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["receipt_image"]["tmp_name"], $target_file)) {
            // Update receipt image in the database
            $query = "UPDATE income SET receipt_image = ? WHERE user_id = ? AND tax_amount IS NOT NULL";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("si", $target_file, $user_id);
            if ($stmt->execute()) {
                echo "The file " . htmlspecialchars(basename($_FILES["receipt_image"]["name"])) . " has been uploaded.";
            } else {
